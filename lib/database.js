...
function getUserDeptQuery() {
    var query = "SELECT d.name, u.name FROM user_dept a JOIN departments d ON (d.id = a.dept_id) JOIN users u ON (u.id = a.user_id);";
...
function searchForTicketsQuery(table, filters, searchParams, includeCompleted, includeExpired, amount) {
    ...
    switch (includeExpired) {
        case "excludeExpired":
            ...
            break;
        case "onlyExpired":
            query += " CURRENT_DATE > due_date AND complete_date IS NOT NULL";
            break;
    }
