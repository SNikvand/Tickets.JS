...
function newTicketQuery( jTicket ) {
    return "INSERT INTO tickets (title, author, author_email, create_date, department, description, priority, due_date"
        + (jTicket.isClient != null ? ", is_client" : "")
        + ") VALUES ( '"
        + jTicket.title + "', '"
        + jTicket.submittedBy + "', '"
        + jTicket.clientEmail + "', "
        ...
        + (jTicket.isClient != null ? ", " + jTicket.isClient : "")
        + " ) RETURNING *;";
}
 ...
 * @param table determines if tickets table or archived tickets table
 * @param filters filter parameters for filtering the ticket search (e.g. department name or assigned user)
 * @param searchParams parameters for searching through keywords in the title or body of a ticket
 * @param includeCompleted to include, exclude, or return only completed tickets
 * @param includeExpired to include, exclude or return only expired tickets
 * @param amount number of tickets retrieved (only relevant at the moment for the front page)
 * @param orderBy db column to order by
 * @param direction direction to order by, ascending or descending
