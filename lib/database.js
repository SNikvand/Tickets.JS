...
function newTicketQuery( jTicket ) {
  var query = "INSERT INTO tickets (hash, title, author, author_email, create_date, department, description, priority, due_date) VALUES ( '" + jTicket.hash + "', '" + jTicket.title + "', '" + jTicket.submittedBy + "', '"
    + jTicket.clientEmail + "', CURRENT_DATE, " + jTicket.dept + ", '" + jTicket.description + "', " + jTicket.priority + ", CURRENT_DATE+" + jTicket.priority * 2 + " ) RETURNING *;";
  return query;
}
...
function modifyTicketQuery( jTicket ) {
  ...
  if ( jTicket.title != undefined ) {
    qParts.push( " title=" + jTicket.title );
  }

  if ( jTicket.description != undefined ) {
    qParts.push( " description=" + jTicket.description );
  }
...
function newUserDeptQuery( deptid, userid ) {
  return "INSERT INTO user_dept VALUES dept_id = " + deptid + ", user_id = " + userid + ";";
}
