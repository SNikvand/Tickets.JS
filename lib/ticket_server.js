...
function handleGetTicket(socket) {
    socket.on('getTicket', function(id_md5) {
        ...
        */
        ...
        // DATABASE FUNCTION: retrieve data for the ticket marked 'id_md5'
...
function handleSetUser(socket) {
    socket.on('setUser', function(id, pass, role) {
        // DATABASE FUNCTION: set user named 'id' with the password 'pass and role 'role'
    });
}
...
function handleGetUser(socket) {
    socket.on('getUser', function(id) {
        ...
        */
        ...
        socket.emit('displayUser', id, role); // should probably add more fields for user
