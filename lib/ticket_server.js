...
function handleSetTicket( socket ) {
  socket.on( 'setTicket', function ( id, hash, title, dept, description, priority, submittedBy, clientEmail, assignedTo, alteredBy, dateCreated, dateDue, dateAltered, isCompleted, isArchive ) {
    ...
    if ( deptid == null ) {
      ...
      dbhelper.queryDatabase( query1, function ( err, result ) {
        ...
        dbhelper.queryDatabase( query2, function ( err, result ) {
          ...
          if ( usernames != null ) {
            assignUsers( usernames, results.rows[0].id );
          }
        } );
      } );
    } else {
...
function assignUsers( usernames, deptID ) {
...
function setTicket( socket, ticket ) {
  ...
  if ( ticket.hash == null ) {
    ...
    ticket.id = hashid.decrypt( ticket.hash );
    dbhelper.queryDatabase( dbhelper.modifyTicketQuery( ticket ), function ( err, result ) {
      console.log( result.rows[0] );
    } );


    var query = "SELECT * FROM " + table + " WHERE id='" + hashids.decrypt( ticket.id ) + "';";
