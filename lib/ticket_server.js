...
exports.listen = function ( server ) {
  ...
  io.sockets.on( 'connection', function ( socket ) {
    ...
    socket.on( 'testReq1', function ( number ) {
      number += 1;
      socket.emit( 'testRes1', number );
    } );

    socket.on( 'testReq2', function ( number ) {
      number += 1;
      socket.emit( 'testRes2', number );
    } );
  } );
};
...
function handleSetTicket( socket ) {
  socket.on( 'setTicket', function ( id, title, dept, description, priority, submittedBy, clientEmail, assignedTo, alteredBy, dateCreated, dateDue, dateAltered, dateCompleted ) {
    if ( id == null ) {
...
function emitTicketList( filters, searchParams, includeCompleted, includeExpired, amount, socket, type ) {
    ...
    pg.connect( dbConfig, function ( err, client, done ) {
      ...
      client.query( unionquery, function ( err, result ) {
          done();
          if ( err ) {
              return console.error( 'error running query', err );
          }
          ...
          switch ( type ) {
              case "admin1":
                  socket.emit( 'displayTicketsAdmin1', result );
                  break;
              case "admin2":
                  socket.emit( 'displayTicketsAdmin2', result );
                  break;
              case "admin3":
                  socket.emit( 'displayTicketsAdmin3', result );
                  break;
              case "manager1":
                  socket.emit( 'displayTicketsManager1', result );
                  break;
              case "manager2":
                  socket.emit( 'displayTicketsManager2', result );
                  break;
              case "manager3":
                  socket.emit( 'displayTicketsManager3', result );
                  break;
              case "ituser1":
                  socket.emit( 'displayTicketsITUser1', result );
                  break;
              case "ituser2":
                  socket.emit( 'displayTicketsITUser2', result );
                  break;
              case "ituser3":
                  socket.emit( 'displayTicketsITUser3', result );
                  break;
              case "view":
                  socket.emit( 'displayTicketsView', result );
                  break;
              default:
...
function handleSetUser( socket ) {
  socket.on( 'setUser', function ( name, pass, role ) {
    ...
    pg.connect( dbConfig, function ( err, client, done ) {
      ...
      // query the database for the id of the user
      client.query( 'SELECT id FROM users WHERE name=' + name + "';", function ( err, result ) {
        ...
        var jUser = { name : name, pass : pass, role : role};

        // if there is no user, create it
        if ( result.rows.length() == 0 ) {
          newUser( jUser );
        } else {
          // add the id to the user info, update the database
          jUser.id = result.rows[0].id;
          updateUser( jUser );
        }
      } );
    } );
