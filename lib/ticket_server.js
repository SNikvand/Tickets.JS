...
function handleDeleteTicket( socket ) {
    socket.on('deleteTicket', function(id, isArchive) {
        ...
        var getId = "SELECT id FROM " + table + " WHERE hash = '" + id + "';"; // select the id based on hash
        var deleteTicket = "DELETE FROM " + table + " WHERE hash = '" + id + "';"; // delete from table

        deleteFromComments(getId, deleteTicket);
        ...
        function deleteFromComments(getId, deleteTicket) {
            pg.connect(dbConfig, function (err, client, done) {
                if (err) {
                    return console.error('error fetching client from pool', err);
                }
                ...
                client.query(getId, function (err, result) {
                    done();
                    if (err) {
                        return console.error('error running query', err);
                    }
                    var deleteComments = "DELETE FROM " + c_table + " WHERE id = " + result.rows[0].id + ";"; // delete from comments table
...
function handleSetReply( socket ) {
  socket.on( 'setReply', function ( ticketId, isArchive, userId, desc ) {
