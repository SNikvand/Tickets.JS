...
exports.authenticateLogin = function(user, pass, callback) {
    ...
    var sendback = {isValid: false, issue: null, userRole: null, userDepts: 'none', lastLogout: null};
    var userid;
    ...
    function getLogoutTime() {
        pg.connect(dbConfig, function (err, client, done) {
            ...
            var query = "SELECT TO_CHAR(last_logout, 'YYYY-MM-DD HH24:MI:SS') FROM users WHERE id = '" + userid + "';";
            ...
            client.query(query, function (err, result) {
                ...
                sendback.lastLogout = result.rows[0].to_char;
                callback(sendback);
            });
        });
    }
...
function handleSetTicket( socket ) {
  socket.on( 'setTicket', function ( id, title, dept, description, priority, submittedBy, clientEmail, assignedTo, alteredBy, dateCreated, dateDue, dateAltered, isCompleted, isArchive ) {
    ...
    dbhelper.queryDatabase( dbhelper.getDeptIdQuery( dept ), function ( err, result ) {
      ...
      results++;
      if ( results == queries ) {
        setTicket( socket, {id : id, title : title, dept : dept, description : description, priority : priority, submittedBy : submittedBy, clientEmail : clientEmail, assignedTo : assignedTo, alteredBy : alteredBy, dateCreated : dateCreated, dateDue : dateDue, dateAltered : dateAltered, isCompleted : isCompleted, isArchive : isArchive } );
      }
    } );
...
function handleDeleteTicket( socket ) {
    socket.on('deleteTicket', function(id, isArchive) {
        ...
        var deleteTicket = "DELETE FROM " + table + " WHERE hash = '" + id + "';"; // delete from table

        deleteFromComments(getHash, deleteTicket);

        function deleteFromTable(deleteTicket) {
            pg.connect(dbConfig, function (err, client, done) {
                if (err) {
                    return console.error('error fetching client from pool', err);
                }

                client.query(deleteTicket, function (err, result) {
                    done();
                    if (err) {
                        console.error('1 error running query', err);
                        return console.error(deleteTicket);
                    }
                });
            });
        }

        function deleteFromComments(getHash, deleteTicket) {
            pg.connect(dbConfig, function (err, client, done) {
                if (err) {
                    return console.error('error fetching client from pool', err);
                }

                // first get the id based on the hash
                client.query(getHash, function (err, result) {
                    done();
                    if (err) {
                        console.error('2 error running query', err);
                        return console.error(getHash);
                    }
                    var deleteComments = "DELETE FROM " + c_table + " WHERE ticket_id = '" + result.rows[0].hash + "';"; // delete from comments table

                    // delete comments for that ticket id
                    client.query(deleteComments, function (err, result) {
                        done();
                        if (err) {
                            console.error('3 error running query', err);
                            return console.error(deleteComments);
                        }

                        // delete ticket
                        deleteFromTable(deleteTicket);
                    });
                });
            });
        }
...
function handleDeleteDept( socket ) {
    socket.on('deleteDept', function(id) {
        ...
        var deleteTickets = "DELETE FROM tickets WHERE department = " + id + ";";
        var deleteTicketsArchive = "DELETE FROM tickets_archive WHERE department = " + id + ";";
        var deleteUserDept = "DELETE FROM user_dept WHERE dept_id = " + id + ";";

        async.series([
                function(callback) {
                    deleteQuery(deleteTickets, callback);
                },
                function(callback) {
                    deleteQuery(deleteTicketsArchive, callback);
                },
                function(callback) {
                    deleteQuery(deleteUserDept, callback);
                }
...
function handleGetReplies( socket ) {
    socket.on('getReplies', function (ticketId, isArchive) {
        ...
        pg.connect(dbConfig, function (err, client, done) {
            ...
            var query = "SELECT c.ticket_id, u.name, c.create_date, c.description FROM " +
                table + " c JOIN users u ON (c.user_id = u.id) ORDER BY create_date desc;";
            client.query(query, function (err, result) {
                // return client to the pool
                done();
                if (err) {
...
function setTicket( socket, ticket ) {
    ...
    if ( ticket.id == null ) {
        ...
        var query = "SELECT * FROM " + table + " WHERE hash='" + ticket.id + "';";
