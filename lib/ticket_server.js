...
function handleGetTicket( socket ) {
  socket.on( 'getTicket', function ( id, isArchive ) {
    ...
    var query = "SELECT t.id, t.title, t.author, t.author_email, t.create_date, d.name AS department, t.description," +
        ...
        if ( err ) {
          return console.error( 'error fetching client from pool', err );
            ...
            for ( var i = 0; i < result.rows.length; i++ ) {
              result.rows[i].hash = hashids.encrypt(result.rows[i].id);
...
function setTicket( socket, ticket ) {
  ...
  if ( ticket.hash == null && ticket.id == null ) {
    ...
    ticket.id = hashids.decrypt( ticket.hash );
